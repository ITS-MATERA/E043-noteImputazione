sap.ui.define([
    ...
    function (BaseController, Filter, MessageBox, FilterOperator, History, DateFormatter) {
        "use strict";
        ...
        return BaseController.extend("project1.controller.aImpegno", {
            ...
            zAttribuitoLiveChange:function(oEvent){
                var self =this,
                    importoAttributoControl = self.getView().byId("importoAttributo"), 
                    newValue =oEvent.getParameters().value,
                    value,
                    tot;

                newValue = newValue.replace(",",".");   
                var tableModel = self.getView().getModel("HeaderNIAssImp");
                tot = 0;
                for(var i=0; i<tableModel.getData().length;i++){
                    var value = tableModel.getData()[i].Attribuito;
                    value = value.replace(",",".");
                    tot = tot + parseFloat(value);
                }
                tot = tot + parseFloat(newValue);                
                var total = self.formatter.convertFormattedNumber(tot.toFixed(2).toString());
                importoAttributoControl.setText(total);    
            },
