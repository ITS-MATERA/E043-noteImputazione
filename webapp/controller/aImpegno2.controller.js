    ...
    function (BaseController, Filter, MessageBox, FilterOperator, History, DateFormatter, JSONModel) {
        ...
        return BaseController.extend("project1.controller.aImpegno2", {
            ...
            loadBeneficiario:function(){
                var self = this,
                    impegni = self.getView().getModel("temp").getProperty("/ImpegniSelezionati");
                
                self.getView().getModel(MODEL_ENTITY).setProperty("/Detail/Lifnr",impegni[0].Lifnr);     
                console.log(impegni);    
                ...
                self.getOwnerComponent().getModel().read(path, {
                    success: function (data) {
                        if(data){
                            self.getView().getModel(MODEL_ENTITY).setProperty("/Detail/Nome",data.ZzragSoc); 
                        }                        
                    },
                    error: function (e) {
                        console.log(e);
            ...
            loadModalitaPagamento:function(){
                ...
                filters.push(new Filter({path: "Lifnr",operator: FilterOperator.EQ,value1: impegni[0].Lifnr}));
                self.getOwnerComponent().getModel().read("/ZdescwelsBniSet", {
                    filters: filters,
                    success: function (data) {
                        if(data && data.results.length>0){
                            self.getView().getModel(MODEL_ENTITY).setProperty('/ZdescwelsBniSet', data.results);
                            self.getView().byId("ModPagamento").setSelectedKey(data.results[0].Zwels);
                            self.getView().getModel(MODEL_ENTITY).setProperty("/Detail/Zwels",data.results[0].Zwels);
                            self.getView().getModel(MODEL_ENTITY).setProperty("/Detail/ZwelsDesc",data.results[0].Zdescwels);
                        }
                    },
                    error: function (error) {
                        var e = error;
            ...
            onDtChange:function(oEvent){
                var self = this,
                    dateValue = self.getView().byId(oEvent.getParameters().id).getDateValue(),
                    dataField = oEvent.getSource().data("dataField");

                self.getView().getModel(MODEL_ENTITY).setProperty("/Detail/"+dataField, dateValue);
            },
            ...
            onSaveButton: function (oEvent) {
                ...
                if( !detail.Lifnr || detail.Lifnr === null || detail.Lifnr === "" ||
                    !detail.Cdc || detail.Cdc === null || detail.Cdc === "" ||
                    ...
                    !detail.Iban || detail.Iban === null || detail.Iban === "" 
                ){
