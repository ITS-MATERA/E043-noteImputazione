    ...
    function (ODataModel, BaseController, MessageBox, Filter, FilterOperator, JSONModel, CoreLibrary, DateFormatter) {
        ...
        return BaseController.extend("project1.controller.wizard", {
            ...
            onInit: async function () {
                ...
                this.getView().setModel(oModelJsonWizardStep1selected, "wizardStep1selected");  
            },
            ...
            viewHeader1: function () {
                var oModelHeader = new sap.ui.model.json.JSONModel();
                // var rows = this.getView().byId("HeaderNIW").getSelectedItems()
                var rows = this.getView().getModel("wizardStep1selected").getProperty("/wizardStep1selected");
                //console.log(rows)
                var lunghezza = rows.length
                //var arrayHeader=[]

                var importoTot = this.getView().getModel("wizardStep1selected").getProperty("/totaleResiduoSelezionati");
            ...
            onPreimpNI: function (oEvent) {
                ...
                if (N_oggSpesa == "") {
                    ...
                    for (var titolo = 0; titolo < this.getView().byId("HeaderNIWstep3").mAggregations.items.length; titolo++) {
                        ...
                        var numeroResiduo = this.getView().byId("HeaderNIWstep3").mAggregations.items[titolo].oBindingContexts.HeaderNIWstep3.oModel.oData[titolo].ZimpoRes;
            ...
            resetWizardStep1selected:function(){
                ...
                self.getView().getModel("wizardStep1selected").setProperty("/wizardStep1selected",[]); 
                self.getView().getModel("wizardStep1selected").setProperty("/totaleResiduoSelezionati",0); 
            ...
            addSelected:function(){
                ...
                for(var i=0; i<selected.length;i++){
                    ...
                    var ZimpoRes = selected[i].getBindingContext("HeaderNIW").getObject().ZimpoRes;
                    total = total + parseFloat(ZimpoRes);
                    // array.push(selected[i].getBindingContext("HeaderNIW").getObject());
                }
                self.getView().getModel("wizardStep1selected").setProperty("/totaleResiduoSelezionati",total);
            },
